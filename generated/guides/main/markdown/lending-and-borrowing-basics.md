...
### Working      
* The protocol maintains a "liquidity pool(s)" where all lenders' assets (liquidity) get pooled.
* There can be a single pool for the entire platform or they might be split based on asset types, or some other strategy
* The borrowers ask for a loan by providing a collateral. Often the loans in DeFi are over collateralized.
* The platform ensure that the borrowed positions remain healthy at all times. If the value of the collateral added by the borrower, drops below a certain threshold, the loans are liquidated. 
* The liquidated funds are sold to independent liquidators at a discounted price. This is done to ensure immediate settlement.
* There can be a  transaction feel added by the platforms and a liquidation fee incase the loans get liquidated.
* After deducting of any platform fees or interest, the interest that is gained from the borrowers, goes to the lenders. Because of the competition, and transparency, plat fees are often quite low.
* The platform might also choose to incentivize either, lender, or borrower, or both, in order to promote a particular side or to promote overall growth of the platform.
